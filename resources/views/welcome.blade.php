...
<body class="antialiased">
    <div class="relative flex items-top justify-center min-h-screen bg-gray-100 dark:bg-gray-900 sm:items-center py-4 sm:pt-0">
        ...
        <div class="card text-center">
            <div class="card-body d-flex flex-column align-items-center">
                <h5 class="card-title mb-4" style="font-size: 2rem;">Rumah Sakit di Daerah Bandung</h5>
                <div id="map"></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        var map = L.map('map').setView([-6.9174163433540725, 107.61568633823215], 13); // Set initial view to a default location (e.g., Jakarta)
        ...
        @foreach ($hospitals as $hospital)
            ...
            marker.bindPopup("<b>{{ $hospital->namahospital }}</b><br>Alamat : {{ $hospital->alamat }}<br>latitude : {{ $hospital->latitude }}<br>Longitude : {{ $hospital->longitude }}<br>Jam Buka : {{ $hospital->jambuka }}<br>Jam Tutup : {{ $hospital->jamtutup }}<br>Layanan : {{ $hospital->layanan }}").openPopup();
        @endforeach
